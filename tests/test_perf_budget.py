...
핵심 계약(v3 — 항목 배치 분할 반영):
  핵심 검토      = 항목 AUDIT_REVIEW_MAX_ITEMS(12)개 묶음당 1콜, 배치 실패는 격리
  자가점검      = 검토 배치 외 LLM 0회 (축 선별·산식·대조는 규칙)
  서면검토 무지적 = 검토 배치만 (검증·합성 생략)
  서면검토 지적  = 검토 배치 + 일괄 문맥검증 1 + 합성 1
...
def _review_batches(report):
    """검토 배치 수 = ceil(판정 항목 수 / 배치 크기) — 호출 예산의 기준."""
    from audit_core.config import get_settings
    n = sum(len(ar.items) for ar in report.axis_results)
    return math.ceil(n / get_settings().AUDIT_REVIEW_MAX_ITEMS)
...
class TestCallBudget(unittest.TestCase):
    def test_self_check_budget_is_review_batches_only(self):
        m = Meter()
        report = _orch(m).self_check("용역", DOC)
        exp = _review_batches(report)
        self.assertEqual(m.calls, ["AxisResult"] * exp)  # 트리아지·산식·대조 LLM 0회

    def test_written_no_finding_skips_verify_and_synth(self):
        m = Meter()
        wr = _orch(m).written_review("용역", DOC)
        exp = _review_batches(wr.report)
        self.assertEqual(m.calls, ["AxisResult"] * exp)  # 검증·합성 전부 생략
        self.assertIsNotNone(wr.opinion)

    def test_written_with_finding_adds_exactly_two_calls(self):
        m = Meter(flag_item="C1")  # C1은 [직접적용] 조문 보유 → 검증 대상
        wr = _orch(m).written_review("용역", DOC)
        exp = _review_batches(wr.report)
        self.assertEqual(m.calls,
                         ["AxisResult"] * exp + ["ContextCheckBatch", "OpinionDraft"])
    ...
    def test_review_timeout_no_retry_partial_result(self):
        ...
        report = _orch(m).self_check("용역", DOC)
        exp = _review_batches(report)
        self.assertEqual(m.calls, ["AxisResult"] * exp)   # 배치당 1회, 재시도 없음
