...
class ReviewReport:
    ...
    # 입력측 결정론 지적(직접경비 산출근거 미첨부 등 — Function 규칙 레인 주입)
    det_notes: list[str] = field(default_factory=list)
...
class Orchestrator:
    ...
    def self_check(
        ...
        should_stop=None,
        evidence_docs: set[str] | None = None,
        extra_rule_notes: list[str] | None = None,
    ) -> ReviewReport:
        """자가점검 DAG (LLM: 축별 검토 / 규칙: 산식 검증).

        evidence_docs(인식된 서류유형 키 집합)가 주어지면, 필요 근거 서류가 전혀
        없는 축은 LLM에 보내지 않고 항목별 '필요 근거 미첨부'로 사전 분류한다
        (rules/evidence_needs.yaml — 어떤 서류가 없어 무엇을 확인 못 하는지 명시).
        ...
        report.det_notes = list(extra_rule_notes or [])
        if report.det_notes:
            for note in report.det_notes:
                progress(f"🔎 [자동 확인] {note[:100]}")
        ...
        # 필요 근거 서류가 없는 축을 사전 분리(EVIDENCE_MISSING) — LLM 미투입
        premarked: dict[str, AxisResult] = {}
        if evidence_docs is not None:
            import yaml as _yaml
            from pathlib import Path as _Path
            _needs = (_yaml.safe_load((_Path(__file__).parent / "rules" /
                                       "evidence_needs.yaml").read_text(encoding="utf-8"))
                      or {}).get("axes", {})
            from audit_core.agents.schemas import AxisItemResult as _AIR
            kept_axes = []
            for a in axes:
                spec = _needs.get(a["axis"])
                if spec and not (set(spec.get("needs", [])) & evidence_docs):
                    note = spec.get("missing_note", "필요 서류 미첨부")
                    premarked[a["axis"]] = AxisResult(axis=a["axis"], items=[
                        _AIR(item_id=it["item_id"], verdict="UNABLE",
                             evidence=f"필요 근거 미첨부 — {note}")
                        for it in a["items"]])
                    progress(f"📋 '{a['title']}' 축 {len(a['items'])}개 항목은 필요 서류가 "
                             f"없어 '근거 미첨부'로 분류했습니다(AI 검토 제외) — "
                             f"{', '.join(spec.get('needs', []))} 중 하나가 필요합니다")
                else:
                    kept_axes.append(a)
            axes_all_order = axes
            axes = kept_axes
        ...
        results = self.reviewer.review_all(axes, llm_doc, should_stop=should_stop,
                                           progress=progress, law_blocks=law_blocks) if axes else []
        if premarked:  # 사전 분류 축을 원래 순서로 합류(침묵 금지 — 항목별 사유 보존)
            by_axis = {r.axis: r for r in results}
            by_axis.update(premarked)
            results = [by_axis[a["axis"]] for a in axes_all_order if a["axis"] in by_axis]
            axes = axes_all_order
    ...
    def written_review(
        ...
        report = self.self_check(
            ...
            evidence_docs=evidence_docs, extra_rule_notes=extra_rule_notes,
        )
...
def format_self_check(report: ReviewReport) -> str:
    ...
    if unable:
        ...
        lines.append(f"### ❔ 판단 불가 {len(unable)}건 — 원인별 구분")
        if causes["NO_REPLY"]:
            ids = ", ".join(it.item_id for _a, it in causes["NO_REPLY"][:12])
            lines.append(
                f"**⚠ AI 미회신 {len(causes['NO_REPLY'])}건** ({ids}) — 시간 제한·모델 실행 "
                f"실패로 판정을 받지 못했습니다. **서류 문제가 아닙니다.** 잠시 후 같은 "
                f"파일로 다시 시도해 주세요.")
            lines.append("")
        if causes["EVIDENCE_MISSING"]:
            lines.append(f"**📋 필요 서류 없음 {len(causes['EVIDENCE_MISSING'])}건** — 아래 서류를 첨부하면 즉시 검토됩니다.")
            by_note: dict[str, list[str]] = {}
            for _a, it in causes["EVIDENCE_MISSING"]:
                note = it.evidence.split(" — ", 1)[-1]
                by_note.setdefault(note, []).append(it.item_id)
            for note, ids in by_note.items():
                lines.append(f"- {note}: {', '.join(ids)}")
            lines.append("")
        if causes["JUDGED_UNABLE"]:
            lines.append(f"**❔ 문서 내 근거 부족 {len(causes['JUDGED_UNABLE'])}건** — AI가 문서를 읽고 판단 근거를 찾지 못한 항목입니다.")
            for _a, it in causes["JUDGED_UNABLE"][:8]:
                lines.append(f"- [{it.item_id}] {it.evidence[:90]}")
            if len(causes["JUDGED_UNABLE"]) > 8:
                lines.append(f"- …외 {len(causes['JUDGED_UNABLE']) - 8}건")
            lines.append("")

    if report.det_notes:
        lines.append("### 🔎 자동 확인 사항 (규칙 검사 — 사람 판단 아님)")
        for note in report.det_notes:
            lines.append(f"- {note}")
        lines.append("")
...
def unable_causes(report: ReviewReport) -> dict[str, list]:
    """판단 불가(UNABLE)를 원인별로 분류 — 추적·표시 공용(로직/모델 문제 구분).

    EVIDENCE_MISSING  필요 근거 서류 미첨부(결정론 사전 분류 — 규칙 레인)
    NO_REPLY          모델이 판정을 회신하지 못함(타임아웃·실행 실패 — 모델/인프라)
    JUDGED_UNABLE     모델이 문서를 보고 '판단 근거 없음'으로 판정(모델 판단)
    """
    out = {"EVIDENCE_MISSING": [], "NO_REPLY": [], "JUDGED_UNABLE": []}
    for axis, it in report.unable():
        if it.evidence.startswith("필요 근거 미첨부"):
            out["EVIDENCE_MISSING"].append((axis, it))
        elif it.evidence.startswith("판정 미회신"):
            out["NO_REPLY"].append((axis, it))
        else:
            out["JUDGED_UNABLE"].append((axis, it))
    return out
...
def format_user_summary(wr: WrittenReview, missing_docs: list[str] | None = None) -> str:
    ...
    n_auto = (len(report.numeric_flags) + len(report.cross_flags)
              + len(report.det_notes) + (1 if report.method_incompat else 0))
    ...
    for note in report.det_notes[:2]:
        head = note.split(" — ", 1)[0]
        src = note.split(" — ", 1)[-1] if " — " in note else ""
        actions.append((head[:60],
                        f"확인 내용: {src[:110]}" if src else f"확인 내용: {note[:110]}",
                        "다음 조치: 해당 산출근거 서류를 첨부해 주세요."))
    if report.method_incompat:
