...
### ⚡ 처리 방식과 속도
결과는 3단계로 나눠 도착합니다 — 기다리지 않으셔도 됩니다.
1. **자동 확인 (몇 초)**: 대상 판정 · 산출내역 검산 · 문서 간 금액 대조 · 빠진 서류 — 먼저 보여드립니다
2. **AI 검토 (약 1~2분)**: 규정 8개 분야를 한 번에 점검
3. **의견서 초안 (`검토` 명령, 최대 5분)**: 감사팀용 검토의견 초안까지
진행 중에는 15초마다 지금 확인한 내용을 실제 근거(파일명·검산 결과)로 알려드리고, 시간이 걸리면 완료된 결과부터 드립니다. 주민번호·전화번호 등 개인정보는 화면에서 가려집니다.
