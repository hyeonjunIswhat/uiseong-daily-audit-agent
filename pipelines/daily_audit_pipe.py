...
※ 결과는 AI 초안이며 최종 판단은 담당자·감사인 몫입니다.
...
※ 기준금액은 의성군 일상감사 규정 별표 원문 기반(감사팀 최종 확인 전)입니다. 결과는 AI 초안이며 최종 판단은 담당자·감사인 몫입니다."""
...
class Pipeline:
    ...
    def _qa_context(self) -> str:
        ...
        lines.append(
            ...
            "- 원가심사와의 관계: 일상감사를 받은 사업은 원가심사를 받은 것으로 본다(규정 제10조④ — 일상감사→원가심사 방향). 원가심사를 먼저 받은 경우의 일상감사 생략 여부는 규정에 명시가 없으므로 감사팀 확인이 필요하다.\n"
        ...
        try:
            ...
            answer = client.chat_text(
                ...
                system=(
                    ...
                    "말고 ① '규정 요지에는 명시가 없다'고 밝힌 뒤 ② '(참고)'를 붙여 일반적인 "
                    "지방계약 실무 방향을 설명하고 감사팀(기획예산과) 확인을 권한다.\n"
                    "- 수치 규율: 금액·비율·기한·조문 항호 번호는 근거에 있는 그대로만. "
                    "예외·특례가 함께 적혀 있으면 같이 언급한다. 없으면 만들지 않는다.\n"
                    "- 문체 규율: 답 끝에 상용구·꼬리 안내문을 붙이지 않는다. 감사팀 문의나 "
